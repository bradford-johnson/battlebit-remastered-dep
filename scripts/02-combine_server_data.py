...
def combine_csv_files(data_directory):
    ...
    for csv_file in csv_files:
        ...
        df["DateTimestamp"] = datetime.strptime(
            datetime_str, "%Y%m%d %H:%M:%S")

        combined_data_frame = pd.concat(
            [combined_data_frame, df], ignore_index=True)
...
def save_combined_data(combined_data_frame, output_file):
    ...
    if os.path.exists(output_file):
        ...
        appended_data_frame = pd.concat(
            [previous_combined_data, combined_data_frame], ignore_index=True)
...
def main():
    data_directory = os.path.join(
        os.path.dirname(__file__), "..", "servers-data")
